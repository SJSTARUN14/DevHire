...
const Register = () => {
    ...
    const { user, isLoading, isError, isSuccess, message, needsVerification, emailForVerification, demoOTP } = useSelector(
        (state) => state.auth
    );
                    ...
                    {!needsVerification ? (
                        ...
                        <form className="mt-8 space-y-6" onSubmit={onVerifyOtp}>
                            <div className="space-y-4">
                                ...
                                {(message?.includes('trouble') || demoOTP) && (
                                    <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                                        ...
                                        <div className="mt-2 text-center text-lg font-mono font-bold text-amber-900 tracking-widest">
                                            {demoOTP || message?.match(/\d{6}/)?.[0] || <span className="text-xs text-amber-600">Please check logs or contact support</span>}
                                        </div>
                                    </div>
                                )}
